...
- If the Singles slot produces the confident read, the information panel shows the one single-battle card. If either Doubles slot produces a confident read, the panel switches to the two double-battle slot cards. Confirmed Pokémon matches are the authoritative layout evidence; raw/unclear OCR text such as `Heatnor-` is shown for OCR Fix context but cannot by itself hold the panel in the wrong layout. The setup buttons save crops and show provisional placeholders for the selected crop type while OCR is running; the next confirmed scan can still switch the card state. Changing Singles/Doubles crop setup invalidates any in-flight OCR worker and queued old results so a scan that started before the layout change cannot switch the panel back afterward.
- Live scanning prioritizes the currently expected slot layout, caps slow no-match OCR attempts per slot, and stops early when it confirms an opposite-layout Pokémon. This lets a readable Singles result such as Roselia commit to the cards before unrelated stale Doubles slots finish slow/noisy OCR.
- Resizing/narrow docking re-renders placeholder cards as well as detected Pokémon cards, and compact double placeholders use shorter titles so the cards do not stay clipped after the panel narrows.
