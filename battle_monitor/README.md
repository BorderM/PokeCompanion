...
- If the Singles slot produces the confident read, the information panel shows the one single-battle card. If either Doubles slot produces OCR text/a confident read, the panel switches to the two double-battle slot cards. The setup buttons only save crops; they do not choose the card state. Each completed scan decides from the names/text seen in that scan, so a stale single card cannot pin the UI after a double-slot name appears, and stale double cards cannot pin it after a single-slot name appears.
